...
\end{equation}
Equation \ref{eq:PSD_Noise_Term} describes an autoregressive process of the first order, with $a_1$ being an AR-1 coefficient \todo{ICE-TEMP: Describe an autoregressive process}.\\
...
\end{equation}
For a more extensive description of the terms and their parameterizations, see Appendix \ref{App:FirnDiffusivity} and \cite[Gkinis et al., 2019]{Gkinis2019}. Here will only be given a short walk-through of the terms:

$m$: molecular weight in [kg],

$p$: saturation vapor pressure over ice in [Pa]with three different calculation methods, see Appendix \ref{App:FirnDiffusivity}, 
...
The Iso-CFM contains three different routines for computing diffusion length profiles, one that is based on a fully analytical solution to Fick's second law, one based on a semi-analytical solution, and on that finds a fully numerical solution. 

\begin{itemize}
	\item \lstinline[columns=fixed]|analytical_hl|:
	\item \lstinline[columns=fixed]|semi_analytical_hl|:
	\item \lstinline[columns=fixed]|numerical_hl|:
\end{itemize}



\section[Temperature Estimation]{Temperature Estimation}
\todo{ICE-TEMP.EST:This entire section needs to be written}
