...
\frame{
	...
	\begin{figure}
		...
		\begin{subfigure}{.4\textwidth}
			\centering
			\includegraphics[width=\linewidth]{TurnerEruptionofSoufriere-Mountains}
			\caption[Turner, Volcano]{\tiny (1815) J. M. W. Turner: "\textit{The Eruption of the Soufriére 	Mountains in the Island of St. Vincent, 30 April 1812}"}
			\label{fig:Turner1}
		\end{subfigure}%
		~
		\begin{subfigure}{.5\textwidth}
			\centering
			\includegraphics[width=\linewidth]{Turnerlancaster-sands.jpg}
			\caption[Diffusion length profile, Site A]{\tiny (1828) J. M. W. Turner: "\textit{Landcaster Sands}"}
			\label{fig:Turner2}
		\end{subfigure}
		%		\caption{A figure with two subfigures}
		\label{fig:Turner}
...
\frame{
	...
	\begin{figure}
		...
		\caption[Combined restored data, Site A.]{The original data plotted along with each estimate of the restored data with diffusion length $\sigma^2 < 0.075$.}
...
\frame{
	\frametitle{Peak Detection SciPy.signal.find\_peaks}
	\begin{figure}
		\centering
		\includegraphics[width=0.8\textwidth]{SiteA_Peaks}
	\end{figure}
}

\frame{
	\frametitle{More Robust Peak Detection }
	\begin{itemize}
		\item Signal representation by piecewise polynomial interpolation
		\item Least squares analytic fitting to cubicwe splines
	\end{itemize}
}
